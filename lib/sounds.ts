...
/** Разблокировать AudioContext после первого жеста (Chrome / Safari). */
...
function playSoundInner(ctx: AudioContext, type: Sfx) {
  ...
  switch (type) {
    ...
    case 'block':
      ...
      tone(ctx, t + 0.04, 520, 0.08, 0.06, 'square')
      break
    case 'dodge':
      noiseBurst(ctx, t, { dur: 0.06, freq: 620, gain: 0.09, q: 0.5 })
      tone(ctx, t, 320, 0.12, 0.05, 'triangle', 180)
      break
    case 'dark':
...
export async function playSoundAsync(type: Sfx): Promise<void> {
  if (!isSoundEnabled()) return
  try {
    const ctx = getAudioContext()
    if (!ctx) return
    if (ctx.state === 'suspended') await ctx.resume()
    playSoundInner(ctx, type)
  } catch {
    /* optional */
  }
}
